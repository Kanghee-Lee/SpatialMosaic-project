...
# from llava.data.lazysuperviseddataset import LazySupervisedDataset
...
class DataArguments:
    ...
    frame_folder: Optional[str] = field(default=None)
...
def load_non_lora_trainables(model, lora_weight_path):
    non_lora_path = os.path.join(lora_weight_path, "non_lora_trainables.bin")
    if not os.path.exists(non_lora_path):
        rank0_print(f"No non_lora_trainables.bin found at {non_lora_path}; skipping non-LoRA weight load.")
        return

    rank0_print(f"Loading non-LoRA trainables from {non_lora_path}...")
    non_lora_trainables = torch.load(non_lora_path, map_location="cpu")
    incompatible = model.load_state_dict(non_lora_trainables, strict=False)

    # Older LLaVA checkpoints are sometimes saved with PEFT prefixes, while
    # base models expect the same tensors without them.
    if len(incompatible.unexpected_keys) == len(non_lora_trainables):
        normalized = {(k[11:] if k.startswith("base_model.") else k): v for k, v in non_lora_trainables.items()}
    ...
    rank0_print(
        "Finished loading non-LoRA trainables "
...
class LazySupervisedDataset(Dataset):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        elif "frames" in sources[0]: ###
            frames_list = sources[0]["frames"]
            scene_name = sources[0]['scene_name']
            frame_root = self.data_args.frame_folder
            frame_file_list = [os.path.join(frame_root, scene_name, 'images', str(f)+'.jpg') for f in frames_list]
            for frame_file in frame_file_list:
                if not os.path.exists(frame_file):
                    print("File {} not exist!".format(frame_file))
            try:
                processor = self.data_args.image_processor
                video = []
                for frame_file in frame_file_list:
                ...
                print(f"Failed to process frames for scene: {scene_name}")
                print(sources[0]["conversations"])
                print(sources[0]["id"])
                return self._get_item(i + 1)
        ...
        has_image = ("image" in self.list_data_dict[i]) or ("video" in self.list_data_dict[i]) or ("frames" in self.list_data_dict[i])
        ...
        if "image" in self.list_data_dict[i]:
            ...
            data_dict["image"] = image
        elif "frames" in self.list_data_dict[i]:
            data_dict["image"] = image
...
def train(attn_implementation=None):
    ...
    if training_args.lora_weight_path and not training_args.lora_enable:
        raise ValueError("--lora_weight_path is only supported together with --lora_enable True.")
    ...
    if training_args.lora_enable:
        ...
        if training_args.lora_weight_path:
            ...
            rank0_print(f"Loading existing LoRA adapters from {training_args.lora_weight_path} for continued training...")
    ...
    if model_args.vision_tower is not None:
        ...
        if training_args.lora_weight_path:
            load_non_lora_trainables(model, training_args.lora_weight_path)
