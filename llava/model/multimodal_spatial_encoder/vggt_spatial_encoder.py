...
class VGGTSpatialConfig(PretrainedConfig):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Union[str, os.PathLike], **kwargs) -> "PretrainedConfig":
        ...
        # get the spatial config dict if we are loading from a wrapped spatial config
...
def prepare_input(pixel_values):
    ...
    # resize to 378x378
    pixel_values = pixel_values.reshape(-1, 3, pixel_values.shape[-2], pixel_values.shape[-1]) # (F_max*B, 3, H, W)
    pixel_values = nn.functional.interpolate(pixel_values, size=(518, 518), mode='bilinear') 
    ...
    def forward(
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast(dtype=views.dtype):
                aggregated_tokens_list, ps_idx = self.vggt.aggregator(views) # 1, 32, 3, 518, 518
...
class VGGTSpatialTower(nn.Module):
    def __init__(self, spatial_tower, spatial_tower_cfg, delay_load=True):
        ...
        dynamic_weights_path = os.path.join(repo_root, 'vggt', default_weights_name) # Construct full path
