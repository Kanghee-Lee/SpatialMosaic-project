    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                ...
                additional_config = {
                    "tie_word_embeddings": getattr(cfg_pretrained, "tie_word_embeddings", True),
                    "use_cache": True,
                }
                if hasattr(cfg_pretrained, "text_config") and getattr(cfg_pretrained.text_config, "vocab_size", None) is not None:
                    additional_config["vocab_size"] = cfg_pretrained.text_config.vocab_size
                elif getattr(cfg_pretrained, "vocab_size", None) is not None:
                    additional_config["vocab_size"] = cfg_pretrained.vocab_size
                if overwrite_config is not None:
                    overwrite_config.update(additional_config)
                    rank0_print(f"Overwriting config with {overwrite_config}")
                    for k, v in overwrite_config.items():
            ...
            if "mixtral" in model_name.lower():
                ...
                if "moe" in model_name.lower() or "A14B" in model_name.lower():
                    ...
                    additional_config = {
                        "tie_word_embeddings": getattr(llava_cfg, "tie_word_embeddings", True),
                        "use_cache": True,
                    }
                    if hasattr(llava_cfg, "text_config") and getattr(llava_cfg.text_config, "vocab_size", None) is not None:
                        additional_config["vocab_size"] = llava_cfg.text_config.vocab_size
                    elif getattr(llava_cfg, "vocab_size", None) is not None:
                        additional_config["vocab_size"] = llava_cfg.vocab_size
                    del kwargs["device_map"]
                    if overwrite_config is not None:
                        overwrite_config.update(additional_config)
                        rank0_print(f"Overwriting config with {overwrite_config}")
